...
@pytest.mark.parametrize(
    ...
    sorted(privacy.REVIEWED_GENERATED_ARTIFACT_SHA256.items()),
)
def test_reviewed_generated_artifact_exceptions_are_exact_hash_pins(
    relative_path: str,
    expected_sha256: str,
) -> None:
    """Allows reviewed generated bytes only; any drift is scanned again."""
    content = (ROOT / relative_path).read_bytes()
    assert hashlib.sha256(content).hexdigest() == expected_sha256
    assert not privacy.public_artifact_byte_findings(content, relative_path=relative_path)

    changed = content + (
        "\nhttps:" + chr(47) * 2 + "drive.google.com/file/d/" + "1" + "A" * 28
    ).encode()
    assert "drive_url" in {
        kind
        for _line, kind in privacy.public_artifact_byte_findings(
