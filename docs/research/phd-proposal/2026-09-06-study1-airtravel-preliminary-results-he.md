...
> **סייג זמינות נוכחי:** חבילת השקיפות בקוד הנוכחי מסמנת את ערכי `C1`/`C2`/`C3` כ-`NOT_AVAILABLE_IN_WORKTREE` עד להרכבת מניפסט פרטי מאומת. הערכים המספריים במסמך היסטורי זה נשמרים כייחוס רטרוספקטיבי ארכיוני בלבד ואינם מחושבים מחדש כאן.
...
| (ג) תווית דיווח | מועמדות לבדיקה אנושית, הנגזרת משכבה (ב) בלבד | 3 אפיזודות מועמדות | — התוצר |
...
**שושלת גרסאות:** הערך היחיד הקשור לראיות הוא `execution_code_sha` — ‏`efe686ac0b13c6e17695b816da7eb0cdd3eadcc1`. ‏`reporting_code_sha` הוא **חותמת הפקת מסמך בלבד**: הוא מתקדם עם כל commit תיעודי ואינו חלק משרשרת הראיות; ערכים ישנים מסומנים `STAMPED_SUPERSEDED`. `airtravel_real_run.py` השתנה לאחר הריצה לצורך receipt-binding, ולכן קוד הביצוע ההיסטורי וקוד הדיווח הנוכחי **אינם byte-identical**. שינויים אלה הם הבחנת provenance ואינם משנים את המספרים ששוחזרו מהאירועים.
...
| בקורפוס זה כל שלוש האפיזודות נופלות באותו צד של הסף | מה הסף הנכון |
