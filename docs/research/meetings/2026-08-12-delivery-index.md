# Aug-12 Supervisor Delivery Index

# מדד מסירה לפגישת 12 באוגוסט

Status: **presentation-ready package for the 2026-08-12 supervisor meeting — not supervisor-approved.**
Every research-question wording remains provisional pending `D-RQ-01`/`D-RQ-02`.

Prepared 2026-08-10 for Prof. Iris Reinhartz-Berger and Prof. Arnon Sturm. All files below derive from the
tracked, machine-derived 2026-08-05 meeting record; nothing here implies prior approval, and every artifact
carries the same claim discipline (EXP-005 = 0 supplied labels / 27 blind / 24 generalization-safe, gate
needs ≥20; medical readiness 0/6; no verbatim transcript quotation; reconstructed wording).

## 1. What to present / מה להציג

| Order | File | Format | Purpose |
| --- | --- | --- | --- |
| 1 | `presentations/VEGO-AI-Aug12-Progress-Review-2026-08-12.pptx` | PPTX (13 slides, English) | **The live progress deck** — screen-share this. Agenda → gap → RQ+SQ1-3 → study map → literature → evidence → plan → decisions → next steps → claim-discipline appendix. |
| 1b | `outputs/aug12-supervisor-delivery-2026-08-10/VEGO-AI-Aug12-Progress-Review-2026-08-12.pdf` | PDF | Portable/backup copy of the same deck (open on any device; email-safe). |
| 2 | `outputs/aug12-supervisor-delivery-2026-08-10/VEGO-AI-Per-RQ-Literature-2026-08-12.xlsx` | Excel (4 sheets) | **Iris's requested literature spreadsheet** — README, Inventory (RQ-tagged), Coverage Gaps, Search Protocol. Drive-ready; replicate its tag column into the native Sheet. |
| 3 | `outputs/aug12-supervisor-delivery-2026-08-10/VEGO-AI-Chapter-3-Gap-and-Research-Questions-2026-08-12.docx` | Word | **Chapter 3, paste-ready** into the Word proposal (Ali works in Word). |

## 2. Supporting documents (repo markdown, sources of record) / מסמכים תומכים

| File | Purpose |
| --- | --- |
| `docs/research/phd-proposal/chapter-3-gap-and-research-questions-draft.md` | Canonical Chapter-3 source (the .docx is rendered from this). |
| `docs/research/meetings/2026-08-05-master-plan.md` | Bilingual master plan — every Iris/Arnon requirement, work breakdown, timeline. |
| `literature/per-rq-literature-map.md` | Per-RQ inventory + coverage-gap analysis (the .xlsx is derived from this). |
| `docs/research/meetings/2026-08-12-walkthrough-outline.md` | Presenter's screen-share script and claim-discipline guardrails. |
| `docs/research/meetings/2026-08-05-tracking.md` | Weekly status tracker. |
| `docs/research/meetings/2026-08-10-work-report.md` | Bilingual work report. |
| `docs/research/meetings/2026-08-05-supervisor-meeting.md` | Canonical machine-derived meeting record (E1–E15, A08-01..09) — the evidence base for all of the above. |

## 3. Integrity manifest (SHA-256) / מניפסט שלמות

Generated files under `outputs/` are gitignored by repo policy and bound here by hash; the PPTX and
markdown are tracked. Bytes and hashes as generated 2026-08-10.

| File | Bytes | SHA-256 | Tracked in git |
| --- | ---: | --- | --- |
| VEGO-AI-Aug12-Progress-Review-2026-08-12.pptx | 312580 | `E0C6BC868F5141768BA48C8D1198247304D97CBF3A8C87EBE0CAB91E9EB71335` | yes (this commit) |
| VEGO-AI-Aug12-Progress-Review-2026-08-12.pdf | 346059 | `526C64AB0A27F6E63E756E662F34BA0529A45BAA43485C54C49001FDB74D1073` | no (gitignored) |
| VEGO-AI-Per-RQ-Literature-2026-08-12.xlsx | 12536 | `A038BB5CF9FFC668A4F825B8732F2343397C862BD8CCA8756F4295C1A7BF1587` | no (gitignored) |
| VEGO-AI-Chapter-3-Gap-and-Research-Questions-2026-08-12.docx | 44643 | `E979D10F8B6AF20844C718D7A07A4E1EBE736138413FA1544DA51A9887B10687` | no (gitignored) |
| chapter-3-gap-and-research-questions-draft.md | 19305 | `90E145921C7CF0FB2E0C8ABEE9D61CD5BEC8D8AAC2CD185FB7FE8E2AB6AD7884` | yes |
| 2026-08-05-master-plan.md | 17699 | `1AE451D5824E026ABC85F9A285DB1E65C9508AB29B3EE164B406D35C7C0677B1` | yes |
| per-rq-literature-map.md | 7232 | `ADEDCBFC078590D86F057C89AD282BB549DEE8B30FD0888CA8D3D8DE7649114B` | yes |
| 2026-08-12-walkthrough-outline.md | 4707 | `132B3B2F0B10ADBF7B026CF46D87D17BDEFF196377C2CE9387F37744A258EE95` | yes |
| 2026-08-10-work-report.md | 8777 | `E1A7EA746EDB245E847A2088D9D196146AA436A0AC57BBC5E2BEB28A64D938C1` | yes |

Regenerate the three `outputs/` binaries with the scripts in the session scratchpad
(`build_deck.js`, `build_xlsx.js`, `build_docx.py`); they are deterministic from the tracked sources
plus the fixed 2026-08-10 timestamp.

## 4. Actions only Ali can do before the meeting / פעולות שרק עלי יכול לבצע

1. **Verify the final RQ/SQ wording** against the saved AI-assistant chat from the 5 Aug call (or re-listen `00:13:07–00:44:37`) — blocks Chapter-3 sign-off (`A08-01`, `D-RQ-01`/`D-RQ-02`).
2. **Share the project Drive** with Iris and Arnon (viewer/comment) — and drop the three `outputs/` files + the PPTX into it.
3. **Check the inbox** for Iris's check-in email (`A08-07`) and reply with real status.
4. Replicate the `rq_tag` column into the native Google Sheet.
5. Paste the Chapter-3 `.docx` content into the Word proposal.
6. One dry run of the walkthrough.

*This index and the three `outputs/` binaries are the deliverable set. The AI environment has no Google
Drive / Gmail / Obsidian connector, so sharing and email (items 2–3) are manual.*
