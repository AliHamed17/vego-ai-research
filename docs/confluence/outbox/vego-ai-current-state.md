...
**Latest 2026-09-08 instrument experiments update (Claude):** Three offline experiments were added and folded into the Study 1 results dossier (now three pages; sections 2, 8, 11): (1) an extended detector envelope of nine deterministic fixture modes — 9/9 conform with exact signal sets, and `WEAK_ALERT`, S1, S2, S3 (empty and null encodings) and S6 are each reached in isolation end-to-end through the protected orchestrator for the first time; (2) instrument robustness on the accepted log — the classification is invariant to event order (500/500 permutations), the value domain is clean, and the S1 signal in one of three episodes rests on Detector-v1's `any` aggregation alone (12 Low / 24 Medium / 3 High answers summarise to Medium under majority or plurality) while no classification does, because that episode stays `STRONG_ALERT` through S7 — an earlier same-day draft that claimed a class dependence was withdrawn after adversarial review; (3) cost calibration — the frozen reserve over-estimates observed spend 6.77x (Study 1B bound) and 2.08x (pilot bound); 16 of 30 enumerated protocols fit USD 6.00 and 5 fit USD 2.00. Provider calls 0; Detector-v1 unchanged; `OPENAI_API_KEY` absent; the USD 6.00 authorisation of 2026-09-08 is unused; decision D8 records what a paid run would require. Work lives in checkout `vego-ai-f2` on `study1/closure-and-study2-prep`. See `docs/research/phd-proposal/2026-09-08-study1-instrument-experiments-addendum.md`.

**Latest 2026-09-08 supervisor-readout update (Codex):** Prepared a six-page Hebrew RTL Study 1 E2E readout, a six-slide Hebrew RTL deck (PPTX/PDF), a compact evidence index, and a Hebrew email draft for Iris and Arnon. The package explains the public-external AirTravel N=4 purposive case set, the Q&A-to-Detector reporting path, the separate Agent-4 mechanism, the archival/descriptive boundary, and the three engineering-only checks. It adds no provider call, corpus change, Detector-v1 change, new scientific denominator, accuracy/benefit claim, or new empirical result. Status: `READY_FOR_SUPERVISOR_REVIEW` after final render QA and CI; independent review remains required.
...
**Last Updated:** 2026-09-06 by Claude (**literature evidence-closure audit** — the four deliverables in `literature/2026-09-06-*.csv` and `docs/research/phd-proposal/2026-09-06-*.md`. Re-verified 54 of 68 proposal references against external records. Searched both adjacent literatures §2.6 names as its likeliest refuters. Assessed 35 candidate refuters, mapped 28 high-priority papers (24 Tier A, 4 Tier B). **All three gaps NARROWED; none refuted.** Chapter 2 claims classified: 22 `SUPPORTED`, 9 `NEEDS_NARROWING`, 2 `LIKELY_FALSE`, 4 `UNVERIFIED`, 0 `REFUTED`. **Chapter 2 was NOT rewritten** — pending reviewer/orchestrator acceptance. Two blocking protocol defects found: the 2026-07-30 register defines five different query families from §4.3 Table 3 (ISS-061), and the QL-05 substrate conjunction structurally prevents the registered generic terms from reaching the BPM and provenance literatures (ISS-062). All four primary databases were ACCESS BLOCKED, so no protocol query was executed and no screening count exists; verification ran via Crossref, OpenAlex, publisher and proceedings records. See ISS-059…ISS-063). Earlier: 2026-09-04 by Codex (Tasks 2–5 technical lane: added the versioned privacy-safe Q&A communication contract, deterministic append-only observer/episode projection, live-event extractor support, and baseline terminology freeze. Offline observer verification passes, but direct wiring into protected `orchestrator.py` remains partial by design; the full suite has one unrelated merge-base hardening-test failure. No live LLM run, API call, human label, frozen-output mutation, or supervisor-document change was made). Earlier the same day, Task 1 local recovery found no original `interaction_log.jsonl`; archived code/logs support conditional historical full-content logging and the 12/30 Q&A baseline remains unchanged. The Q&A escalation detection study remains descriptive and human-validation pending. Earlier: 2026-09-03 by Codex (new supervisor direction makes **Q&A ESCALATION DETECTION STUDY** the active milestone: read-only extraction of inter-agent questions, confidence/evidence observability, transparent alert scaffold, and blind human-review materials; no validation run or performance claim). The Agent-C score reconstruction/C2 bridge remains valid later-stage technical evidence and the 111-versus-114 C2 discrepancy is deferred. The supervisor-facing operational plan now starts with zero-cost `interaction_log.jsonl` recovery, uses a one-setting feasibility rerun, and keeps `ANSWER_NOT_PERSISTED` terminology with no manual labeling in the current plan. Earlier: 2026-09-03 by Codex (strict one-page paired-condition human-intervention experiment). Earlier: 2026-09-02 by Claude (the supervisor call narrowed the preliminary study to ONE descriptive WHEN/WHERE escalation study over the frozen run; EXP-045/046 provide its descriptive basis). Earlier: 2026-08-24 by Claude (delivered the strict proposal review and repaired the pre-existing CI-red build chain; see ISS-039/040/041). Earlier still: 2026-08-14, transcribed and analyzed the 2026-08-12 Iris/Arnon call; see the August 12 checkpoint below and `docs/agent-memory/session-log.md`.
...
| 2026-09-08 | Study 1 E2E Hebrew supervisor readout | READY FOR SUPERVISOR REVIEW / DOCUMENTATION ONLY | Six-page RTL PDF, six-slide RTL PPTX/PDF, evidence index, and email draft explain the public-external AirTravel N=4 purposive sample, Q&A-event-to-Detector reporting flow, separate Agent-4 path, archival descriptive results, and engineering-only checks. No new run, denominator, Detector change, accuracy/benefit claim, or provider call. Independent review remains required. |
| 2026-09-08 | Study 1 instrument experiments (extended envelope truth table, accepted-log robustness, cost calibration) | COMPLETE — ENGINEERING / DESCRIPTIVE ONLY, 0 PROVIDER CALLS | Nine fixture modes conform 9/9 and reach every class and every isolable branch (`WEAK_ALERT`, S1, S2, S3 both encodings, S6) end-to-end for the first time; the accepted run's classification is invariant to event order (500/500) and the S1 signal in one of three episodes rests on the `any` aggregation alone while no classification does (S7 preserves it); the frozen reserve over-estimates observed spend 6.77x / 2.08x and 16 of 30 protocols fit USD 6.00. Dossier now three pages (sections 2, 8, 11); addendum `2026-09-08-study1-instrument-experiments-addendum.md`; D8 added. USD 6.00 authorisation unused; `OPENAI_API_KEY` absent. |
...
- **2026-09-08 (Claude):** to exercise the USD 6.00 authorisation: set `OPENAI_API_KEY` in the execution environment (presence only is checked), close pilot gate 4 (independent review) and gate 6 (frozen model `gpt-5.6-luna`, ceiling, caps), preregister exactly one reserve-bound menu row from the 2026-09-08 addendum section 4 (decision D8), then run with `BudgetGuard` at USD 6.00. Until then only zero-cost instrument experiments run. Candidate next zero-cost experiments: extend the truth table to mixed-confidence multi-answer episodes, and add a per-call ledger to the real-run harness so maxima and per-episode cost stop being `NOT_AVAILABLE`.
...
| ISS-060 | 2026-09-08 | Claude (instrument experiments) | Medium | Medium | Low | Open / human-gated | The USD 6.00 authorisation of 2026-09-08 cannot be exercised: `OPENAI_API_KEY` is absent from the execution environment, pilot gate 4 (independent review) is outstanding, gate 6 (frozen model, ceiling, caps) is unconfirmed, and no proven-bound menu row is preregistered. Only zero-cost instrument experiments were run (addendum 2026-09-08). | Set the credential in the environment, close gates 4 and 6, preregister exactly one reserve-bound menu row (decision D8), then run with `BudgetGuard` at USD 6.00 and the frozen model. |
...
- Decision: Amend the search protocol **before** execution rather than executing as registered and amending afterwards. Executing as written would breach the proposal's own rule that expressions are not changed after execution, and would produce a "required target not reached" record for both literatures §2.6 names.
