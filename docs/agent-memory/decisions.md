...
|---|---|---|---|
| 2026-08-19 | Literature index format: generated awesome-list, not hand-authored | Active | `literature/README.md` is now generated by `scripts/build_awesome_literature_index.py` from `verified-research-corpus-2026-08-12.json`; format modelled on `github.com/HenryPengZou/Awesome-Human-Agent-Collaboration-Interaction-Systems`, which Iris shared in the 2026-08-05 call chat in reply to Ali sharing Zou et al. 2026 (ACL Findings, `2026.findings-acl.1811`). Never hand-edit the README; edit the corpus JSON and rerun the script. |
| 2026-08-19 | `output/` (singular) stays a separate root from `outputs/` (plural) | Active | Both are gitignored. `output/` is still actively referenced by 4 supervisor-package scripts (`build_supervisor_preread.py`, `build_thesis_review_manifest.py`, `publish_supervisor_package.py`, `validate_supervisor_decision_package.py`); merging it into `outputs/` requires editing those scripts' hard-coded paths and was out of scope for the 2026-08-19 clutter cleanup, which only removed the two genuinely stray root files. |
