...
**Last Updated:** 2026-09-03 by Codex (added the strict one-page paired-condition human-intervention experiment requested for Iris: three frozen Cheers/ParkWise cases, explicit Condition A/B, observable-versus-manual trigger status, bounded controlled input, reference evidence, and every outcome left `To be measured`; the generated PDF is exactly one A4 page and remains ignored/local. No effectiveness, accuracy, superiority, or approval claim is made). Earlier: 2026-09-02 by Claude (the supervisor call narrowed the preliminary work to ONE descriptive WHEN/WHERE escalation study over the frozen run; EXP-045/046 provide its descriptive basis). Earlier: 2026-08-24 by Claude (delivered the strict proposal review and repaired the pre-existing CI-red build chain; see ISS-039/040/041). Earlier still: 2026-08-14, transcribed and analyzed the 2026-08-12 Iris/Arnon call; see the August 12 checkpoint below and `docs/agent-memory/session-log.md`.
