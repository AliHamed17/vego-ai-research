...
| Study 2A ON/OFF preparation | Ready for independent review; run authorization pending | Draft PR #40 contains the separate ON/OFF configs, schema, deterministic preparation manifest, Hebrew preregistration/readiness docs, and disabled-by-default offline harness. The latest CI run passed all six jobs. | Review the baseline definition and parity contract; issue a fresh explicit authorization before any provider-backed condition run. |
