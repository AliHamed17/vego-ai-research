...
def technical_appendix(analysis: dict[str, Any], manifest: dict[str, Any], validation: dict[str, Any] | None, cards: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        f"| Requests counted by guard / ledger rows | | | {analysis['accounting']['receipt_requests']} / {analysis['accounting']['ledger_requests']} |",
        f"| Unrecorded in-flight requests (charged at full reserve) | | | {analysis['accounting']['unrecorded_in_flight_requests']} |",
        f"| Spend upper bound USD | | | {analysis['accounting']['spend_upper_bound_usd']:.6f} |",
        "",
        f"Accounting note: {analysis['accounting']['note']}.",
        "",
...
def budget_spend_receipt(analysis: dict[str, Any], budget_doc: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"| Unrecorded in-flight requests / spend upper bound (USD) | | {analysis['accounting']['unrecorded_in_flight_requests']} / {analysis['accounting']['spend_upper_bound_usd']:.6f} |",
...
def claims_table(analysis: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("Mapping-row counts as a cross-condition quality measure", f"ON {on['mapping_rows_total']} rows over {on['completed']} artifacts against Domain-Advisor guidelines; OFF {off['mapping_rows_total']} rows over {off['completed']} artifacts against self-derived guidelines", analysis["evidence_class"], "FORBIDDEN as quality; PERMITTED as structural description with the construction caveat"),
...
def executive_summary(analysis: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Cost: ON {on['cost_usd']:.4f} USD, OFF {off['cost_usd']:.4f} USD; recorded total {analysis['budget']['actual_cost_usd']:.4f} USD (upper bound {analysis['accounting']['spend_upper_bound_usd']:.4f} USD with {analysis['accounting']['unrecorded_in_flight_requests']} in-flight request(s) charged at full reserve) against a 5.8241 USD reservation and a 6.00 USD ceiling.",
        ...
        f"- Artifact structure differs by construction: ON artifacts carry {min(on.get('mapping_rows_per_case') or [0])}-{max(on.get('mapping_rows_per_case') or [0])} mapping rows per case"
        + (f" (the Domain Advisor produced {analysis['on_reference_guideline_count']} reference guideline(s))" if analysis.get("on_reference_guideline_count") is not None else "")
        + f" and {min(on.get('uncovered_per_case') or [0])}-{max(on.get('uncovered_per_case') or [0])} uncovered fragments (total {on['uncovered_total']}: {fmt_counts(on.get('fragment_labels_total'))}); OFF, which receives no guideline list, derived {min(off.get('mapping_rows_per_case') or [0])}-{max(off.get('mapping_rows_per_case') or [0])} guidelines per case and flagged {min(off.get('uncovered_per_case') or [0])}-{max(off.get('uncovered_per_case') or [0])} fragments (total {off['uncovered_total']}: {fmt_counts(off.get('fragment_labels_total'))}). Row counts are therefore not comparable as a quality measure.",
        "",
