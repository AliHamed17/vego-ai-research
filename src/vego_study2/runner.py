...
from .schemas import SchemaValidationError, validate_named, validate_receipt, validate_result
...
class Study2Runner:
        ...
        async def one(case: dict[str, Any]) -> None:
            ...
            for role in roles:
                ...
                if not usage["valid"]:
                    ...
                    return
                valid, reason = validate_case_output(case["case_id"], payload, condition=condition)
                if not valid:
    ...
    def _qa_events(self, fixture_mode: str) -> tuple[int, int, list[dict[str, Any]]]:
        ...
        recorder = QACommunicationRecorder(run_id=self.config["run_id"])
        for case in self.config["case_ids"]:
            ...
            for round_index in range(1, rounds + 1):
                ...
                question = recorder.emit_question(
                    ...
                    scope="fixture",
                    case_id=case,
                    question_text=f"fixture-question:{case}:{round_index}",
                    round_index=round_index,
                )
                recorder.emit_answer(
                    question=question,
                    answer_text=f"fixture-answer:{case}:{round_index}",
                    answer_confidence="Medium",
                    answer_evidence=f"fixture-evidence:{case}:{round_index}",
                )
    ...
    async def run_off(self, *, fixture_mode: str = "two_rounds") -> dict[str, Any]:
        # Keep the paired fixture mode explicit in both condition receipts;
        # OFF still emits no Q&A regardless of the selected mode.
        return await self._run_condition("VEGO_AI_OFF", fixture_mode=fixture_mode)

    async def run_both(self, *, fixture_mode: str = "two_rounds") -> dict[str, Any]:
        on = await self.run_on(fixture_mode=fixture_mode)
        off = await self.run_off(fixture_mode=fixture_mode)
...
def validate_case_output(
    case_id: str, payload: Any, *, condition: str | None = None
) -> tuple[bool, str | None]:
    ...
    if payload["schema_version"] != "study2-condition-output-v1":
        return False, "OUTPUT_SCHEMA_VERSION_INVALID"
    if condition is not None and payload["condition"] != condition:
    ...
    try:
        validate_named(payload, "study2-condition-output-v1.schema.json")
    except SchemaValidationError:
        return False, "OUTPUT_SCHEMA_INVALID"
    return True, None
