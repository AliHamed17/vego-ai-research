# Render the English and Hebrew detection-baseline PDFs, then rasterise every page so the layout
...
$render = Join-Path $env:TEMP "detection-baseline-render"
$profileRoot = Join-Path $env:TEMP "detection-baseline-chrome"
...
foreach ($lang in @("en", "he")) {
    $src = Join-Path $figures "detection-baseline-$lang.html"
    $out = Join-Path $docs "2026-09-09-VEGO-AI-Detection-Baseline-$($lang.ToUpper()).pdf"
...
for lang in ('EN', 'HE'):
    name = f'2026-09-09-VEGO-AI-Detection-Baseline-{lang}.pdf'
