"""Prepare-only by default. Exact execution requires an owner-issued, hash-bound grant.
Observer logs contain hashes/lengths; protected pipeline outputs contain full state.
All execution outputs remain private under an exact ignored external_data child.
No execution or authorization is implied by successful preparation.
"""
...
FROZEN: dict[str, Any] = {
    ...
    "runtime_files": {
        "domain_description/description.md": (
            "96bc8a6fbf2c2fdd93592fdbf6fac7c2b9db403494fe2d5a45e0a2bcbf0167e2",
            1477,
        ),
    ...
    "allowed_output_prefixes": ("external_data/",),
}
...
def check_runtime_pack(runtime_root: Path) -> dict[str, Any]:
    ...
    from airtravel_preflight_contract import no_links

    try:
        no_links(runtime_root)
        for path in runtime_root.rglob("*"):
            no_links(path)
    except ValueError:
        return {"name": "runtime_pack", "status": "FAIL", "problems": ["runtime link refused"]}
...
def check_protected_files(repo_root: Path) -> dict[str, Any]:
    problems = []
    from airtravel_preflight_contract import protected_hashes

    try:
        protected_hashes(repo_root)
    except (ValueError, OSError, __import__("subprocess").CalledProcessError):
        problems.append("policy-derived protected inventory differs from green main")
...
def check_output_dir(output_dir: Path | None, repo_root: Path) -> dict[str, Any]:
    if output_dir is None:
        return {
            ...
            "status": "FAIL",
            "problems": [],
            "note": "not supplied in prepare-only mode",
        }
    try:
        from airtravel_preflight_contract import output_path

        output_path(output_dir, repo_root)
...
def check_provider_disabled(runtime_root: Path) -> dict[str, Any]:
    ...
    if config_path.is_file():
        ...
        if (
            config.get("description_path") != "domain_description/description.md"
            or config.get("candidate_models_dir") != "candidate_models"
            or config.get("runtime_files") != sorted(FROZEN["runtime_files"])
        ):
            problems.append(
                "runtime configuration must resolve exactly to frozen description/candidates"
            )
    else:
    ...
    return {
        ...
        "fake_client": "scripts/airtravel_local_observer.RecordingFake(two_rounds)",
    }


def prepare_only(
    runtime_root: Path, runtime_archive: Path, output_dir: Path | None, repo_root: Path = REPO_ROOT
) -> dict[str, Any]:
    ...
    from airtravel_preflight_contract import BASE, git

    try:
        if git(repo_root, "rev-parse", "origin/main") != BASE:
            raise ValueError("base moved")
        if output_dir is not None:
            relative = output_dir.resolve().relative_to(repo_root.resolve()).as_posix()
            if not git(repo_root, "check-ignore", "--", relative):
                raise ValueError("output not ignored")
        checks.append({"name": "base_and_ignore", "status": "PASS", "problems": []})
    except (ValueError, OSError, __import__("subprocess").CalledProcessError):
        checks.append(
            {
                "name": "base_and_ignore",
                "status": "FAIL",
                "problems": ["base/output policy rejected"],
            }
        )
...
def execute_preflight(
    ...
    authorization_grant: Path | None = None,
) -> dict[str, Any]:
    """Authorized future path: run the unchanged protected orchestrator with the fake client."""
    if authorization_grant is None:
        raise PreflightGateError("authorization packet and separate grant required")
    from airtravel_preflight_contract import authorize
    ...
    try:
        ...
        raise PreflightGateError("authorization grant rejected") from exc
    from airtravel_preflight_execution import execute_verified
...
def main() -> int:
    ...
    parser.add_argument("--authorization-grant", type=Path)
    ...
    from airtravel_preflight_contract import receipt_path

    if args.receipt:
        try:
            if args.output_dir is None:
                raise ValueError("output required for receipt")
            receipt_path(args.receipt, args.output_dir)
        except ValueError:
            parser.error("receipt must be inside the exact output directory")

    if args.execute and args.prepare_only:
        parser.error("--execute and --prepare-only are mutually exclusive")
    if args.execute:
        if not args.authorization_grant or not args.authorization_packet:
            parser.error("--execute requires --authorization-packet and --authorization-grant")
